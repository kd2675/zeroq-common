...
1. 서브 에이전트를 적절히 호출한다. 병렬 가능 에이전트라도 기본은 1개이며, 메인이 직접 판단한 경우에만 최대 3개까지 병렬로 늘린다
2. 서브 에이전트의 제안을 검토한다
...
- "이미 구현된 것처럼 보인다", "diff가 작다", "시간이 부족하다", "테스트가 느리다" 같은 이유로 coder / reviewer / tester 단계를 생략하면 안 된다
