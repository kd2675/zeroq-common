...
if [[ "${TARGET_ENVIRONMENT}" == "operating" ]]; then
  if [[ "${STOCK_V4_OPERATING_ALLOW_MATURITY_PROMOTION:-}" != "YES" ]]; then
    printf 'FAIL operating maturity promotion requires STOCK_V4_OPERATING_ALLOW_MATURITY_PROMOTION=YES\n' >&2
    exit 1
  fi
  if [[ "${STOCK_MYSQL_REPLAY_SCHEMA}" != "STOCK_SERVICE" ]]; then
    printf 'FAIL operating maturity promotion requires exact STOCK_SERVICE schema\n' >&2
    exit 1
  fi
