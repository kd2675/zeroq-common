...
const files = {
  ...
  autoProfileCohortPanel: read("app/supply-demand/admin/AdminAutoProfileCohortPanel.tsx"),
  autoProfileConfigForm: read("app/supply-demand/admin/AdminProfileConfigFormPanel.tsx"),
...
const checks = [
  ...
  ["automatic-participant queries expose profile aggregates instead of individual account filters", includesAll(files.queryLayer, [
    "autoParticipantProfileOverviews: (options?: { activityScope?: string; profileTypes?: string[] })",
    "autoProfileCohorts: () =>",
    "[...(options?.profileTypes ?? [])].sort(),",
  ]) && !includesAny(files.queryLayer + files.stockApi + files.types, [
    "autoParticipantOverviews:",
    "getAutoParticipantOverviews",
    "getAutoParticipantSymbolConfigs",
    "AutoParticipantLifecycleScope",
  ])],
  ["root page routes to primary stock pages", includesAll(files.home, [
    ...
    "내 주식",
    "수요와 공급 주문 체결",
  ]) && !files.home.includes('activityScope: "ALL"') && includesAll(files.publicNavigation, [
    'href: "/trade"',
    'href: "/orders"',
    'href: "/portfolio"',
    'href: "/research"',
  ...
  ["scaled-market rebase pins automatic-market, LP, and institution capacity before activation", includesAll(
    ...
    [
      ...
      "V5 자동참여자의 실행계좌·주문·수량은 실제 원장값으로 기록하며",
      "대표 인구는 행동 빈도에만 반영하고 종목 목표·현금·보유·주문수량에는 곱하지 않습니다.",
      "수용 불가",
    ],
  )],
  ["V5 calibration audits actual submission, execution, and order-count bands", includesAll(
    ...
    [
      ...
      "종목별 실제 거래량 부족분",
      "내부 실행계좌의 주문·수량을 실제 원장값으로 집계합니다.",
      "대표 인구는 프로필별 관심 빈도에만 반영하며 주문수량과 완료장 관찰값에는 곱하지 않습니다.",
  ...
  ["admin automatic-participant surface is profile-only", includesAll(
    files.autoProfileCohortPanel + files.supplyDemandAdmin + files.stockApi + files.types,
    [
      "15만 대표 인구 · 실행 샤드 계약",
      "15만 개인투자자의 행동은 프로필별 확률분포로 생성하고",
      ...
      "/api/stock/v1/markets/auto-market/profile-overviews",
    ],
  ) && !includesAny(files.supplyDemandAdmin + files.stockApi + files.types, [
  ...
  ["admin recurring cash settings are profile-level and allow day or longer intervals only",
    sameSet(recurringCashIntervalOptions, ["DAY", "MONTH", "YEAR"])
      && includesAll(files.autoProfileConfigForm, [
        "RECURRING_CASH_INTERVAL_UNIT_OPTIONS",
        "recurringDepositAmount",
        "recurringDepositIntervalValue",
        "recurringDepositIntervalUnit",
