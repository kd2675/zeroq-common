...
const checks = [
  ...
  ["scaled-market rebase pins automatic-market, LP, and institution capacity before activation", includesAll(
    ...
    [
      ...
      "대표인구 100배는 활성 계약의 표본 주문수량과 코호트",
      "종목 목표·현금·보유·방향별 잔여량을 넘을 수 없습니다.",
      "수용 불가",
    ],
  )],
  ["V4 calibration separates represented order sizing from completed-day observations", includesAll(
    ...
    [
      ...
      "종목별 실제 거래량 부족분",
      "실행 {formatNumber(calibration.engineParticipantCount)}명은",
      "엔진 계정 불일치",
      "표본 주문수량과 대표 코호트 판단에 반영하되",
      "완료장 관찰 수치에는 다시 곱하지 않습니다.",
      "다음 정책 묶음 변경 가능",
      "목표 거래대금 범위",
    ],
  )],
  ...
  ["admin market flow preserves the overall page and switches five role pages without per-tab refetch", includesAll(
